...
/**
 * 儿童端专用日志工具
 * 双输出：Logcat (tag: ParentalControl_Child) + 本地文件
 *
 * 日志格式: [yyyy-MM-dd HH:mm:ss.SSS][Module][LEVEL] message
 *
 * 使用前需调用 ChildLogUtil.init(context)
 */
object ChildLogUtil {

    const val LOGCAT_TAG = "ParentalControl_Child"
    ...
    /**
     * 初始化日志系统
     * 建议在 Application 或 MainActivity 早期调用
     */
    fun init(context: Context) {
        ...
        synchronized(this) {
            ...
            try {
                ...
                isInitialized = true
                writeLog("ChildLogUtil", "INFO", "儿童端日志系统初始化 | logDir=$logDir")
            } catch (e: Exception) {
                // 回退到内部存储
                ...
                isInitialized = true
                Log.e(LOGCAT_TAG, "日志系统回退到内部存储: $logDir")
            }
        }
    }
    ...
    private fun ensureInit() {
        if (!isInitialized) {
            Log.w(LOGCAT_TAG, "ChildLogUtil 未初始化，仅输出到 Logcat")
        }
    }
    ...
    private fun getLogFile(): File? {
        ...
        if (today != currentDate || currentLogFile == null) {
            synchronized(this) {
                if (today != currentDate || currentLogFile == null) {
                    currentDate = today
                    currentLogFile = File(dir, "child_log_$today.txt")
                }
            }
        }
